...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalTimerId = setTimeout(function () {
    var scroll = Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["calcScroll"])();
    document.body.style.marginRight = "".concat(scroll, "px");
    Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["openModal"])('.popup-consultation', modalTimerId);
  }, 60000);
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])('.button-design', '.popup-design', '.popup-design .popup-close', modalTimerId);
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])('.button-consultation', '.popup-consultation', '.popup-consultation .popup-close');
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])('.fixed-gift', '.popup-gift', '.popup-gift .popup-close');
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["openByScroll"])('.fixed-gift');
...
var accordion = function accordion(triggersSelector) {
  ...
  var hideSlide = function hideSlide(n) {
    btns.forEach(function (btn, i) {
      if (i !== n) {
...
var burger = function burger(menuSelector, burgerSelector) {
  var upElem = document.querySelector(menuSelector),
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = 'Пожалуйста выибирете размер и материал картины';
...
var forms = function forms() {
  var form = document.querySelectorAll('form'),
      ...
      upload = document.querySelectorAll('[name="upload"]'); //Получаем элементы. Form и input получаем все что есть.
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // навешиваем обработчик на формы и отменяем стандартное поведение.

      var statusMessage = document.createElement('div'); // создаем элемент для сообщения пользователю.

      statusMessage.classList.add('status'); // добавляем класс элементу.

      item.parentNode.appendChild(statusMessage); // помещаем его в родителя формы

      item.classList.add('animated', 'fadeOutUp'); // добавим форме 2 класса. Animated - позволяет анимировать эл. fadeUpout - 
      ...
      var statusImg = document.createElement('img'); // создаем в переменной тег img.

      statusImg.setAttribute('src', message.spinner); // устанавливаем тегу источник из нашего объекта с сообщениями пользователю.

      statusImg.classList.add('animated', 'fadeInUp'); // добавляем спинеру классы с анимацией.

      statusMessage.appendChild(statusImg); // помещаем наш спиннер в блок statusMessage.

      var textMessege = document.createElement('div'); // создаем текстовое сопровождение спиннеру.

      textMessege.textContent = message.loading; // помещаем в него текст из объекта с сообщениеями.

      statusMessage.appendChild(textMessege); // добавляем на страницу, к спиннеру текстовый блок.

      var formData = new FormData(item); // с помощью ф-ции конструктора создаем объект Form Data, который собирает данные из форм.

      var api;
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question; // если обертка формы(ближайший класс прородителя) = .popup-design, от путь к нашему серверу
      ...
      .finally(function () {
        ...
        setTimeout(function () {
          ...
          item.style.display = 'block'; // возвращаем форме block

          item.classList.remove('fadeOutUp'); // удаляем анимированное скрытие

          item.classList.add('fadeInUp'); //добавим аним. появление.
        }, 4000);
...
/*!**********************************!*\
  !*** ./src/js/modules/modals.js ***!
  \**********************************/
/*! exports provided: default, openModal, closeModal, calcScroll, openByScroll */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "openByScroll", function() { return openByScroll; });
/* harmony import */ var core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! core-js/modules/web.dom-collections.for-each */ "./node_modules/core-js/modules/web.dom-collections.for-each.js");
/* harmony import */ var core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_0__);
...
function bindModal(triggerSelector, modalSelector, closeSelector, modalTimerId) {
  ...
  modalTrigger.forEach(function (item) {
    item.addEventListener('click', function (e) {
      ...
      document.body.style.marginRight = "".concat(scroll, "px"); // добавим отступ на ширину скролла что бы не прыгала страница при открытии
    });
  });
  close.addEventListener('click', function () {
    ...
    document.body.style.marginRight = "0px"; // уберем отступ на ширину скролла что бы не прыгала страница при закрытии
  });
...
function calcScroll() {
  ...
  var div = document.createElement('div'); // создаем произвольный элемент.

  div.style.width = '50px';
  div.style.height = '50px';
  div.style.overflowY = 'scroll';
  div.style.visibility = 'hidden'; // задаем ему стили - размеры - не выжны, важно div.style.overflowY = 'scroll'

  document.body.appendChild(div); // добавляем элемент на страницу

  var scrollWidth = div.offsetWidth - div.clientWidth; // Создаем переменную ширина скролла. Расчитываем ее: берем наш блок и узнаем его полную ширину.
  ...
  div.remove(); // удаляем технический объект.

  return scrollWidth; // возвращаем из функции ширину скролла.
}
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    ...
    block.querySelectorAll('p:not(.sizes-hit)').forEach(function (p) {
      // 'p:not(.sizes.hit)' - позволяет выбрать все параграфы, кроме тех у которых есть класс sizes.hit
...
var scrolling = function scrolling(upSelectior) {
  var upElem = document.querySelector(upSelectior);
  window.addEventListener('scroll', function () {
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles').then(function (res) {
      ...
      return statusMessage.textContent = 'Что то пошло не так....';
    });
    this.remove();
  });
